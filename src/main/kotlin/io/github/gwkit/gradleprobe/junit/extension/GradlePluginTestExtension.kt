...
/**
 * JUnit 5 extension that sets up Gradle plugin test infrastructure.
 *
 * This extension:
 * - Copies the test project from resources to a temporary directory
 * - Processes build files based on the `kts` flag (deletes either Groovy or Kotlin DSL files)
 * - Injects properties annotated with [RootProjectDir], [GradleRunnerInstance], and [ProjectFile]
 *
 * @see GradlePluginTest
 */
class GradlePluginTestExtension : TestInstancePostProcessor {
    ...
    private fun resolveExistingFile(rootProjectDir: File, relativePath: String): File {
        return rootProjectDir.resolve(relativePath)
            .takeIf { it.exists() }
            ?: error("File $relativePath not found in $rootProjectDir")
    }
